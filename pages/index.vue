      ...
      <ul>
        <li v-for="lesson in week" :key="lesson.id">
          ...
          <nuxt-link
            ...
            :to="`${lesson.course}/lessons/${lesson.slug}`"
            >{{ lesson.label }} - {{ lesson.title }}</nuxt-link
          >
  ...
  async asyncData({ $content, $dayjs }) {
    ...
    lessons = lessons.map((item, index) => {
      ...
      item.week =
        week +
        Math.floor(
          (itemDate.getTime() - firstMon.getTime()) / 24 / 60 / 60 / 1000 / 7
        );
      ...
      item.label = `${item.course.split('-')[0].toUpperCase()} ${
        item.course.split('-')[1]
      } Day ${item.day}`;
